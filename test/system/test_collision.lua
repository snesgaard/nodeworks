...
T("test_collision", function(T)
    ...
    collision
        .register(id, nw.spatial(0, 0, 10, 10))
        .register(id2, nw.spatial(100, 0, 10, 10))
    ...
    T("move", function(T)
        ...
        T:assert(event.swap())
        T:assert(#event.get(nw.event_type.move) == 1)
    end)
    ...
    T("world_hitbox_offset", function(T)
        local id3 = {}
        collision
            .register(id3, nw.spatial(1, 2, 3, 4))
            .warp_to(id3, 100, 200)
    ...
    T("implicit_garbage_collection", function(T)
        local id3 = {}
        collision.register(id3, nw.spatial(0, 0, 1, 1))
    ...
    T("flipping", function(T)
        collision.warp_to(id, 0, 0)
        collision.flip_to(id, true)
        T:assert(stack.get(nw.component.mirror, id))
        T:assert(nw.spatial(collision.get_world_hitbox(id)) == nw.spatial(-10, 0, 10, 10))
        collision.flip(id)
        T:assert(not stack.get(nw.component.mirror, id))
        T:assert(nw.spatial(collision.get_world_hitbox(id)) == nw.spatial(0, 0, 10, 10))
    end)

    T("oneway_in_y", function(T)
        local id3 = {}

        collision.register(id3, nw.spatial(0, 100, 10, 10))
