...
T("input_remap", function(T)
    ...
    nw.system.input_remap.set_keyboard_map(world, keyboard_map)

    T("keyboard", function(T)
        for key, input in pairs(keyboard_map) do
            world("keypressed", key)
            T:assert(world:singleton()[input] == 1)
            world("keyreleased", key)
            T:assert(world:singleton()[input] == 0)
        end
    end)

    local gamepad_map = {x = "left", y = "right"}
    nw.system.input_remap.set_gamepad_button_remap(world, gamepad_map)
